...
def group_semantic_keywords(keywords, distance_threshold=0.5):
    if not keywords:
        return {}
...
def process_articles(articles, top_n=10):
    ...
    for month, group in df.groupby("month"):
        ...
        for cid, kws in clusters.items():
            ...
            rep_row = sub_df.sort_values("score", ascending=True).iloc[0]
            cluster_summary.append({
                "cluster_id": int(cid),
                "keywords": kws,
                "representative": rep_row["keyword"],
                "score": float(rep_row["score"])
            })

        cluster_summary = sorted(cluster_summary, key=lambda x: x["score"])[:top_n]
...
if __name__ == "__main__":
    ...
    clusters_by_month = process_articles(articles, top_n=10)

    with open("clustered_keywords_top_month.json", "w", encoding="utf-8") as f:
        json.dump(clusters_by_month, f, ensure_ascii=False, indent=2)

    print("✅ Exported clustered_keywords_top_month.json")
